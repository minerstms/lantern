...
export async function fetchAdminUserRow(db, username) {
  ...
  return db
    .prepare(
      `SELECT username, display_name, first_name, last_name, honorific, public_display_name, staff_id, email, role, student_character_name, teacher_id, mtss_student_id, is_active, updated_at, must_change_password, password_reset_at, password_reset_by FROM lantern_pilot_accounts WHERE lower(trim(username)) = lower(trim(?))`
    )
