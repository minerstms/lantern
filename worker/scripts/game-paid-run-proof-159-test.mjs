...
const expectedIds = ['avatar-match', 'clickrush', 'handbook-trivia', 'lantern-live-trivia', 'local-history-trivia', 'memory', 'minecart-switch', 'nuggetHunt', 'orbit-lock', 'reaction', 'seven-habits-trivia', 'srp-safety-trivia', 'tower'];
if (catalogIds.join() === expectedIds.sort().join()) ok('33. all thirteen catalog games remain the production set');
...
async function main() {
...

  {
    ...
    if (r.status === 400 && r.json.error === 'invalid_game') ok('19/34. invalid / unregistered lab game fails');
...

  {
    const state = { accounts: { '20889': me } };
    const env = makeEnv(state);
    const r = await postRecord(env, cookie, { game_name: 'Unregistered Minecart', score: 5, run_id: 'run-mine' });
    if (r.status === 400 && r.json.error === 'invalid_game') ok('35. unregistered game cannot submit');
    else bad('35 unregistered', r);
  }
