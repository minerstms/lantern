...
if (games.length === 10) ok('Play catalog has 10 games including Stack Lab');
else bad('catalog count changed', games.length);

if (sandbox.LANTERN_GAME_CATALOG.getGameById('tower') && sandbox.LANTERN_GAME_CATALOG.getGameByName('Stack Lab')) {
  ok('Stack Lab is registered in the Play catalog');
} else bad('Stack Lab missing from LANTERN_GAME_CATALOG');
...
originalNine.forEach(function (name) {
  if (sandbox.LANTERN_GAME_CATALOG.getGameByName(name)) ok('existing game intact: ' + name);
  else bad('existing game missing', name);
});
...
const postedMissing = await bridge.submitLeaderboardScore({
  gameId: 'tower',
  gameName: 'Stack Lab',
  score: 150,
  floors: 6,
...
if (
  postedMissing.ok === false &&
  postedMissing.error === 'missing_run_id' &&
  postedMissing.leaderboardPosted === false &&
  fetchCalls.length === 0
) {
  ok('score adapter refuses production submit without paid run_id');
} else bad('submitLeaderboardScore missing run_id', postedMissing);
...
if (
  ...
  posted.submitted.game_name === 'Stack Lab' &&
  ...
  String(fetchCalls[0].init.body).indexOf('character_name') === -1
) {
  ok('score adapter posts Lantern game name without client character_name');
...
const noId = await (function () {
  ...
  return bridge.submitLeaderboardScore({ gameName: 'Stack Lab', score: 10, character_name: 'DONOR_USER' });
})();
