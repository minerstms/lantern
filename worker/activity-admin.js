/**
 * Prompt #257C — Lantern Activities admin (global missions + games registry).
 */
...
/** Global perm_* mission metadata (capability + default kind). */
export const GLOBAL_MISSION_REGISTRY = [
  ...
  { id: 'perm_grade_reflection', kind: 'event', placement: PLACEMENT_MISSION, placementLocked: true },
...
export function isGlobalMissionId(id) {
  return String(id || '').trim().startsWith('perm_');
}
...
function missionKindFromRow(row) {
  const reg = registryForMissionId(row && row.id);
  if (reg) return reg.kind;
  if ((row && row.submission_type) === 'confirmation') return 'event';
  return 'submission';
}

function missionRowToActivity(row, origin, placementOverride) {
  const reg = registryForMissionId(row.id);
  const kind = missionKindFromRow(row);
  ...
  return {
    ...
    kind,
    global: isGlobalMissionId(row.id),
    active: row.active !== 0,
    ...
    suggested_reward: suggestedMissionReward(minChars),
    student_preview: formatMissionStudentPreview(minChars, reward, requireImage),
    card_image_url: row.card_image_r2_key ? (String(origin || '').replace(/\/$/, '') + '/api/news/image?key=' + encodeURIComponent(row.card_image_r2_key)) : null,
...
export async function patchGlobalMissionActivity(db, missionId, body, updatedBy, origin) {
  ...
  if (!isGlobalMissionId(id)) return { ok: false, error: 'not_global_mission' };
  const row = await db.prepare('SELECT id, allows_image FROM lantern_missions WHERE id = ?').bind(id).first();
  if (!row) return { ok: false, error: 'not_found' };
  const reg = registryForMissionId(id);
  const kind = reg ? reg.kind : 'submission';
  const updates = [];
  const bindings = [];
  ...
  if (body.min_characters !== undefined) {
    const validated = validateOrdinaryMissionMinCharacters(body.min_characters, kind);
