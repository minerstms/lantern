...
function missionRowToActivity(row, origin, placementOverride, rewardModeOverride) {
  ...
  const rewardMode = normalizeRewardMode(rewardModeOverride);
  ...
  return {
    ...
    student_preview: formatMissionStudentPreview(minChars, reward, requireImage, rewardMode),

    admin_economy_preview: formatRewardModeAdminPreview(reward, rewardMode),
...
export async function buildActivitiesAdminPayload(db, origin) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    const rewardMode = await getMissionRewardMode(db, row.id);

    missions.push(missionRowToActivity(row, origin, placement, rewardMode));
...
export async function patchGlobalMissionActivity(db, missionId, body, updatedBy, origin) {
  ...
  if (body.reward_mode !== undefined) {
    ...
    const saved = await setMissionRewardMode(db, id, body.reward_mode, updatedBy);
  ...
  return { ok: true, activity: missionRowToActivity(fresh, origin, placement, rewardMode) };
