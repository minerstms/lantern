...
import { evaluateSchoolSchedule, resolveUntilSchoolCloseInstant } from './school-schedule.js';
import {
...
async function handleAdminRoutes(request, url, path, env, cors) {
  ...
  if (request.method === 'GET' && path === '/api/admin/avatar/status') {
    ...
    const resolved = await resolveCanonicalAvatarState(db, characterName, {
      candidates: collectAvatarLookupCandidates(target.username, characterName),
      includePending: true,
    });
    ...
    const activeImage = resolved.currentKey
      ? buildAvatarImageUrl(origin, resolved.currentKey, profile && profile.updated_at)
      : null;
    const approvedImage = resolved.approvedKey
      ? buildAvatarImageUrl(origin, resolved.approvedKey, approved && (approved.approved_at || approved.created_at))
      : null;
    const stagedImage = pending && pending.image_key
      ? buildAvatarImageUrl(origin, pending.image_key, pending.created_at)
      : null;
    const canSetCurrent = !!(
      targetRole === 'student' &&
      !mediaRestricted &&
      resolved.approvedKey &&
      resolved.approvedKey !== resolved.currentKey
    );
    return jsonResponse(
      {
        ok: true,
        ...
        current_avatar_key: resolved.currentKey,
        ...
        approved: !!approved,
        approved_id: approved && approved.id ? String(approved.id) : null,
  ...
  if (request.method === 'POST' && path === '/api/admin/avatar/activate') {
    ...
    const pending = await db
      ...
      .bind(characterName)
      .first();
    const requestedId = String(body.submission_id || '').trim();
    let approved = null;
    if (!pending || !pending.image_key) {
      if (requestedId) {
        const row = await db
          ...
          .bind(requestedId)
          .first();
        if (row && String(row.character_name || '') === characterName && String(row.status || '') === 'approved') {
          approved = row;
        }
      }
      if (!approved) {
        approved = await loadLatestApprovedAvatarSubmission(db, [characterName, target.username]);
      }
    }
    if ((!pending || !pending.image_key) && (!approved || !approved.image_key)) {
    ...
    if (pending && pending.image_key) {
      ...
      imageKey = approved.image_key;
    }
    await writeCurrentAvatarKey(db, characterName, imageKey, now);
    const origin = new URL(request.url).origin;
    return jsonResponse(
      {
        ok: true,
        ...
        active_image: buildAvatarImageUrl(origin, imageKey, now),
        current_avatar_key: imageKey,
        staged: false,
        activated: true,
        set_current: true,
...
/**
 * School-access signal computation and central gate live in `./school-access-decision.js`
 * (Prompt #34 foundation, Prompt #262 precedence + D1 enforcement setting).
 */
...
async function handleClassAccessRoutes(request, url, path, env, cors) {
  ...
  if (request.method === 'GET' && path === '/api/class-access/state') {
    ...
    const accessStatus = await buildAccessEnforcementStatus(db, env, new Date());
    const schedule = accessStatus.schedule;
    const scheduleEnforcementEnabled = accessStatus.enforcement_enabled;
    ...
    if (mode === 'simulation') {
      ...
      return jsonResponse({
        ...
        enforcement_enabled: accessStatus.enforcement_enabled,
        enforcement_source: accessStatus.enforcement_source,
        currently_inside_lock_window: accessStatus.currently_inside_lock_window,
        effective_enforcement_active: accessStatus.effective_enforcement_active,
        schoolwide_override_active: !!(eventOverride && eventOverride.qualifyingAccess),
        lock_window: accessStatus.lock_window,
        timezone: accessStatus.timezone,
        individualGrant,
        deviceGroupAccess,
        eventOverride,
        qualifyingAccess,
    ...
    const stateMeta = {
      ...
      enforcement_enabled: gate.enforcementEnabled,
      enforcement_source: accessStatus.enforcement_source,
      currently_inside_lock_window: gate.currentlyInsideLockWindow,
      effective_enforcement_active: gate.effectiveEnforcementActive,
      schoolwide_override_active: !!(eventOverride && eventOverride.qualifyingAccess),
      lock_window: accessStatus.lock_window,
      timezone: accessStatus.timezone,
      individualGrant,
    ...
    if (gate.allowed) {
      const grantExpiresAt =
        ...
        null;
      const bypassReasons = ['event_override', 'device_group_unlock', 'individual_grant'];
      const accessState = bypassReasons.indexOf(gate.reason) >= 0
        ? 'live_student_has_valid_access'
        : 'live_outside_school_hours';
    ...
    return jsonResponseWithCookies({
      ...
      tokenValid: false,
      gate_reason: gate.reason,
      message: 'Lantern access is currently limited during the school day.',
      ...stateMeta,
    }, 200, cors, stateCookies);
  }
  ...
  if (request.method === 'GET' && path === '/api/avatar/status') {
    ...
    if (account && !pilotAccountRequiresChangePassword(account)) {
      if (isTeacherLike(account.role)) {
        ...
        const ownKeys = collectAvatarLookupCandidates(
          avatarCharacterNameForPilotAccount(account),
          ...avatarCandidatesFromPilotAccount(account)
        ).map((k) => k.toLowerCase());
        if (ownKeys.indexOf(characterName.toLowerCase()) >= 0 || ownKeys.indexOf(String(resolved.resolvedCharacterName || '').toLowerCase()) >= 0) {
          canSeePending = true;
        }
      }
    }
    let pending = null;
    if (canSeePending) {
      const pendingState = await resolveCanonicalAvatarState(db, characterName, {
        candidates: aliasCandidates,
        includePending: true,
      });
    ...
    const activeImage = advertiseActive ? buildAvatarImageUrl(origin, resolved.publicImageKey, stamp) : null;
