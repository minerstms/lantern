...
(function (global) {
  ...
  function submitLeaderboardScore(opts) {
    ...
    // Prompt #128 / #159 — never send client-authoritative character_name. Session cookie
    // is the identity. run_id is required and comes from Lantern paid-start, not the donor.
    var body = {
      game_name: gameName,
      score: score,
    ...
    if (!runId) {
      return Promise.resolve({ ok: false, error: 'missing_run_id', ignoredDonorFields: ignoredDonorFields, leaderboardPosted: false });
    }
    body.run_id = runId;
