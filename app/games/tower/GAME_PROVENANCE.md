...
**Status:** Lab prototype only. Not on `origin/main`. Not listed on Play.
**Audit date:** 2026-08-13
**Auditor lane:** RED — commercial / IP documentation. No gameplay, scoring, Nuggets, Play, auth, D1, deploy, or merge-to-main changes.
**This document does not give legal advice.** Classifications are conservative product-risk judgments from available repository and public evidence.
...
## 11. Prompt #132 overlay (lab engineering — not a new legal audit)

This section records engineering follow-up on branch `cursor/tower-donor-lab-4f8f`. It does **not** replace §§1–10 and is **not** a commercial-clearance GO.

| Change | Purpose |
|--------|---------|
| [`asset-slots.json`](./asset-slots.json) | Inventory of every donor media slot and the runtime source for that slot. |
| [`apply-asset-overlay.mjs`](./apply-asset-overlay.mjs) | Copies Lantern replacements into `assets/` when present; otherwise copies marked **temporary donor placeholders** for canvas gameplay only. |
| `assets/` | What the hosted game actually loads (`./assets/…` with no `<base href="./donor/">`). |
| `lantern-assets/` | Drop-in directory for Lantern-owned art/audio using the **same filenames** as the donor engine. |
| Hosted `index.html` chrome | Lantern CSS/HTML for loading, title, Start, and game-over. Donor title art, START bitmap, modal bitmaps, loading GIF, and BMQB logos are **not** referenced. |
| Audio | `soundOn: false`; `playBgm` is a no-op; runtime `assets/*.mp3` / `*.ogg` are silent placeholders. Caketown and donor SFX remain only under `donor/assets/` for provenance. |
| Canvas font default | `wenxue` → `Arial` in `donor/src/utils.js` and the matching default in `donor/dist/main.js`. Webfont files stay omitted. |
| Working title | User-facing lab chrome uses **Stack Lab** (temporary). No product name has been selected. See [`NAME_CANDIDATES.md`](./NAME_CANDIDATES.md). |

**Production merge is not recommended** while donor gameplay art remains in `assets/` as placeholders, while Caketown/SFX files remain in the vendor tree, while BMQB logo files remain on disk, and while the commercial merge gate in [`COMMERCIAL_MERGE_GATE.md`](./COMMERCIAL_MERGE_GATE.md) is incomplete.
