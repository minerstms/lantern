...
- Same-origin iframe + `lantern-adapter.js` (paid-start gated; parent posts leaderboard; 10 floors = qualifying win and run completion).
...
- Keyboard (Space / Enter / ArrowDown), pointer, and touch all place the swinging floor.
- Score is clamped to 0–2500 before parent submission.
...
## 8. Score formula (deterministic, bounded)

Internal game id remains `tower`. Public working title is **Stack Lab**.

Defaults from MIT engine `addScore` (`donor/src/utils.js`):

- `successScore = 25`
- `perfectScore = 25`
- Consecutive perfect streak `P` resets to 0 on any non-perfect land or miss.

On each successful floor:

```
score += 25 + (25 * P)
```

where `P = previous consecutive perfects + 1` if this floor is perfect, else `P = 0`.

Worked examples:

| Event | P after | Score added | Running score |
|---|---|---|---|
| Floor 1 ordinary | 0 | 25 | 25 |
| Floor 2 perfect | 1 | 50 | 75 |
| Floor 3 perfect | 2 | 75 | 150 |
| Floor 4 ordinary | 0 | 25 | 175 |

A 10-floor all-perfect run:

```
sum_{k=1..10} (25 + 25k) = 25 * 10 * 13 / 2 = 1625
```

A 10-floor run with no perfects: `25 * 10 = 250`.

**Server allowlist:** `scoreMin = 0`, `scoreMax = 2500` (headroom above 1625; rejects negatives and oversized payloads).

Client clamp and parent Game Player clamp both use 2500 before `POST /api/leaderboards/record`.

---

## 9. Win / end conditions

- **Win / completion:** 10 successful stacked floors. The run stops. Qualifying win awards +1 Nugget through existing `game_win` (server delta exactly +1, session identity, same `run_id`).
- **Loss:** 3 misses (HP depleted) before 10 floors. No win award. Score still records.
- Restart is **Play Again** → shared `tryPlay` → a new 1 Nugget paid `run_id`. The iframe does not free-replay in `lanternPlay=1` mode.
